...
def test_semantic_memory_manager():
    ...
    for i, memory_text in enumerate(test_memories, 1):
        ...
        success, message, memory_id = manager.add_memory(memory_text, memory_db, USER_ID)
    ...
    for query in search_queries:
        ...
        if results:
            ...
            for memory, similarity in results:
                print(f"   📋 {similarity:.3f}: '{memory.memory}' (topics: {memory.topics})")
        else:
    ...
    for text1, text2 in similarity_tests:
        similarity = manager.duplicate_detector._calculate_semantic_similarity(text1, text2)
