...
def initialize_smolagents_system():
    ...
    if USE_WEAVIATE:
        ...
        if success:
            ...
            from .core.memory import vector_store as vs, weaviate_client as wc
            weaviate_client = wc
            vector_store = vs
    ...
    return smolagents_agent, query_knowledge_base, store_interaction, clear_knowledge_base
...
def create_smolagents_web_app():
    ...
    agent, query_kb_func, store_int_func, clear_kb_func = initialize_smolagents_system()

    # Create Flask app
    app = create_app()

    # Register routes with smolagents-compatible interface
    register_routes(
        app,
        agent,
        logger_instance,
