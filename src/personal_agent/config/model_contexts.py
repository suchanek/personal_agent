...
from .settings import get_env_var, OLLAMA_URL
...
async def query_ollama_model_info(model_name: str, ollama_url: str = OLLAMA_URL) -> Optional[Dict]:
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.debug("Failed to get model info for %s: HTTP %d", model_name, response.status_code)
            return None
...
def get_env_override_for_model(model_name: str) -> Optional[int]:
    ...
    if override_value:
        try:
            ...
            logger.warning("Invalid context size override for %s: %s", model_name, override_value)
    ...
    if general_override:
        try:
            ...
            logger.warning("Invalid default context size override: %s", general_override)
    
    return None

async def get_model_context_size(model_name: str, ollama_url: str = OLLAMA_URL) -> Tuple[int, str]:
    """
    ...
    if env_override:
        logger.info("Using environment override for %s: %d tokens", model_name, env_override)
    ...
    try:
        ...
        if model_info:
            ...
            if ollama_context:
                logger.info("Detected context size from Ollama API for %s: %d tokens", model_name, ollama_context)
    ...
    if name_context:
        logger.info("Extracted context size from model name %s: %d tokens", model_name, name_context)
    ...
    if normalized_name in MODEL_CONTEXT_SIZES:
        ...
        logger.info("Found context size in database for %s: %d tokens", model_name, db_context)
        return db_context, "database_lookup"
    
    # 5. Use default fallback
    default_context = MODEL_CONTEXT_SIZES["default"]
    logger.warning("Using default context size for unknown model %s: %d tokens", model_name, default_context)
    return default_context, "default_fallback"

def get_model_context_size_sync(model_name: str, ollama_url: str = OLLAMA_URL) -> Tuple[int, str]:
    """
...
def add_model_to_database(model_name: str, context_size: int) -> None:
    ...
    logger.info("Added model %s to context database: %d tokens", normalized_name, context_size)
