#!/usr/bin/env python3
"""
Enhanced LightRAG Document Deletion Script

This script provides comprehensive document deletion capabilities:
...
class EnhancedLightRAGDocumentManager:
    def __init__(self, base_url: str = "http://localhost:9621", 
                 storage_path: str = "/Users/Shared/personal_agent_data/agno/rag_storage"):
    ...
    def check_docker_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'running': lightrag_service is not None and lightrag_service.get('State') == 'running',
    ...
    def delete_document_api(self, doc_id: str) -> bool:
        ...
        try:
            response = requests.delete(f"{self.base_url}/documents/delete_document",
                                     json={"doc_id": doc_id})
            ...
            if response.status_code in [200, 204]:
                ...
                print(f"❌ API deletion failed for {doc_id}: {response.status_code} - {response.text}")
                return False
    ...
    def delete_documents_comprehensive(self, doc_ids: List[str], 
        ...
        if use_persistent:
            ...
            print(f"✅ Storage deletion complete: {results['storage_deleted']}/{len(doc_ids)}")
            print()
        ...
        if verify:
            ...
            if self.check_server_status():
                ...
                print(f"✅ Verification complete: {verified_deleted}/{len(doc_ids)} confirmed deleted")
            else:
    ...
    def delete_documents_by_status_comprehensive(self, status: str,
                                               use_persistent: bool = False,
                                               restart_server: bool = False,
                                               verify: bool = False,
        ...
        # Confirmation
        if confirm:
            response = input(f"Delete these {len(docs)} documents with status '{status}'? (y/N): ")
        ...
        return self.delete_documents_comprehensive(doc_ids, use_persistent, restart_server, verify)


def main():
    import argparse
    
    parser = argparse.ArgumentParser(description="Enhanced LightRAG Document Management")
    ...
    parser.add_argument("--status", action="store_true",
                       help="Show server and Docker status")
    parser.add_argument("--delete-processing", action="store_true", 
                       help="Delete all processing documents")
    parser.add_argument("--delete-failed", action="store_true", 
                       help="Delete all failed documents")
    parser.add_argument("--delete-status", type=str, 
                       help="Delete all documents with specific status")
    ...
    parser.add_argument("--verify", action="store_true",
                       help="Verify deletion after completion")
    ...
    if args.status:
        ...
        print(f"LightRAG Server: {'🟢 Running' if server_running else '🔴 Not responding'}")
        
        # Docker status
        docker_status = manager.check_docker_status()
        print(f"Docker Services: {'🟢 Running' if docker_status['running'] else '🔴 Not running'}")
        ...
        # Storage path
        storage_exists = os.path.exists(args.storage_path)
    ...
    if use_persistent and not restart_server:
        print("⚠️  Warning: Persistent deletion without server restart may not take effect immediately")
    ...
    if args.delete_processing:
        ...
        print("\nQuick examples:")
        print("  python delete_lightrag_documents_enhanced.py --status")
        print("  python delete_lightrag_documents_enhanced.py --list")
        print("  python delete_lightrag_documents_enhanced.py --delete-processing")
        print("  python delete_lightrag_documents_enhanced.py --delete-processing --persistent --restart-server --verify")
        return
