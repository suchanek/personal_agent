...
from src.personal_agent.core.semantic_memory_manager import MemoryStorageResult, MemoryStorageStatus
...
class TestAgentMemoryManager(unittest.TestCase):
    ...
    def test_direct_search_memories_with_agno_memory(self):
        ...
        result = self.manager.direct_search_memories("test query", limit=5, similarity_threshold=0.5)
    ...
    def test_restate_user_fact(self):
        """Test restating user facts from first to third person."""
        test_cases = [
    ...
    def test_restate_user_fact_word_boundaries(self):
        ...
        test_cases = [
            ...
            ("imagination is key", "imagination is key"),  # "I" in "imagination" should not change
        ]
...
class TestAgentMemoryManagerAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_store_user_memory_success(self):
        ...
        with patch.object(self.manager, 'store_graph_memory', new_callable=AsyncMock) as mock_store_graph:
            mock_store_graph.return_value = "✅ Graph memory synced successfully"
            
            result = await self.manager.store_user_memory("Test memory content", ["test"])
            ...
            mock_store_graph.assert_called_once_with(
                "Test memory content", ["test"], "mem_123"
            )
    ...
    async def test_store_user_memory_topic_handling(self):
        ...
        with patch.object(self.manager, 'store_graph_memory', new_callable=AsyncMock):
            ...
            await self.manager.store_user_memory("Test content", "topic1, topic2, topic3")
            ...
            # Test list
            await self.manager.store_user_memory("Test content", ["list_topic1", "list_topic2"])
            call_args = self.mock_memory_manager.add_memory.call_args
    ...
    async def test_query_memory_generic_queries(self):
        """Test that generic queries delegate to get_all_memories."""
        with patch.object(self.manager, 'get_all_memories', new_callable=AsyncMock) as mock_get_all:
            mock_get_all.return_value = "All memories result"
    ...
    async def test_get_all_memories(self):
        ...
        self.mock_memory_manager.get_all_memories.return_value = [mock_memory1, mock_memory2]
    ...
    async def test_update_memory(self):
        ...
        self.mock_memory_manager.update_memory.return_value = (True, "Memory updated successfully")
        
        result = await self.manager.update_memory("mem_123", "Updated content", ["new_topic"])
    ...
    async def test_update_memory_failure(self):
        ...
        self.mock_memory_manager.update_memory.return_value = (False, "Memory not found")
    ...
    async def test_clear_all_memories(self, mock_session_class):
        ...
        self.mock_memory_manager.clear_memories.return_value = (True, "Local memories cleared")
        ...
        mock_get_response.json.return_value = {
            "statuses": {
                "indexed": [{"id": "doc1"}, {"id": "doc2"}]
            }
        }
        
        # Mock deleting documents
    ...
    async def test_get_memory_tools_disabled_memory(self):
        ...
        tools = await manager.get_memory_tools()
        
        self.assertEqual(tools, [])
    
    async def test_get_memory_tools_no_agno_memory(self):
        """Test getting memory tools when agno_memory is not initialized."""
        manager = AgentMemoryManager(
            user_id="test_user",
            storage_dir="/tmp/test",
            enable_memory=True
    ...
    async def test_get_memory_tools_success(self):
        """Test getting memory tools successfully."""
        tools = await self.manager.get_memory_tools()
        
        # Should return a list of tool functions
        self.assertIsInstance(tools, list)
        self.assertGreater(len(tools), 10)  # Should have many memory tools
        
        # Check that tools have proper names
