...
### Added
- **User Home Directory Property for Multi-User Isolation**: Implemented `user_home_dir` property in [`PersonalAgentConfig`](src/personal_agent/config/runtime_config.py:695) to provide proper multi-user data isolation. This property returns the patient's isolated home directory (`user_storage_dir`) instead of the system user's home, ensuring that all "home" directory references in a multi-user therapeutic context point to the patient's isolated space under `/Users/Shared/personal_agent_data/agno/{user_id}/`. The property is fully integrated into `ConfigSnapshot`, `to_dict()`, and all filesystem tools. See [ADR-101](./refs/adr/101-user-home-directory-normalization.md) for complete architectural details.
- **Comprehensive User Home Dir Test Suite**: Added 27 new tests in [`test_user_home_dir_property.py`](tests/core/test_user_home_dir_property.py) covering property behavior, ConfigSnapshot integration, filesystem tool integration, multi-user isolation, security requirements, and documentation. All tests pass, validating proper implementation of the new property and its integration throughout the system.
- **Comprehensive Filesystem Tools Test Suite**: Added extensive test coverage for filesystem tools including normalization behavior, multi-user context safety, path expansion security, edge cases, and regression prevention. New test files: [`test_filesystem_tools_normalization.py`](tests/core/test_filesystem_tools_normalization.py) with 23 test classes covering all aspects of the fix, and [`test_personal_agent_tools.py`](tests/tools/test_personal_agent_tools.py) for general filesystem and system tools functionality.

### Fixed
- **Filesystem Tools Natural Language Normalization**: Fixed critical bug where natural language directory references like "current directory" were interpreted literally as directory names instead of path shortcuts. The filesystem tools now normalize common phrases: "current directory"/"current dir"/"here"/"this directory" → `.` (current working directory), and "home"/"home directory"/"home dir" → `config.user_home_dir` (patient's isolated home). This prevents accidental creation of literal directories named "current directory" and ensures proper multi-user data isolation. Implemented in both [`PersonalAgentFilesystemTools.create_and_save_file`](src/personal_agent/tools/personal_agent_tools.py:295-306) and MCP-based [`create_and_save_file`](src/personal_agent/tools/filesystem.py:351-375). See [ADR-101](./refs/adr/101-user-home-directory-normalization.md) for architectural details.
- **Multi-User Home Directory Security**: Eliminated raw `~` expansion in filesystem tools that was causing patient files to be saved to the system user's home directory instead of their isolated space. All tilde (`~`) expansions now use `config.user_home_dir`, ensuring that `~/file.txt` creates files in the patient's isolated directory (`/Users/Shared/personal_agent_data/agno/{user_id}/file.txt`) rather than the system administrator's home (`/Users/egs/file.txt`). This resolves a critical security issue in multi-user therapeutic contexts where patient data isolation is essential.
