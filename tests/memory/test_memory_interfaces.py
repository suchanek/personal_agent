...
async def test_memory_interfaces():
    ...
    try:
        ...
        result = await agent.store_user_memory(
            "I love testing new memory interfaces", topics=["testing", "development"]
        )
        print(f"   Result: {result.status.name} - {result.message}")

        # Test 2: List all memories
        print("\n2. Testing list_memories()...")
        memories_list = await agent.list_all_memories()
        print(
            f"   Found memories: {len(memories_list.split('📝')[1].split('\\n')) if '📝' in memories_list else 0}"
        )
        ...
        memory_methods = [
            ...
            if "memory" in method.lower() and not method.startswith("_")
        ]
