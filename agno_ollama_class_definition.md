# Agno Ollama Class Definition

This document contains the complete class definition for the Ollama class from the agno library (version 1.7.5).

**Source File**: `.venv/lib/python3.12/site-packages/agno/models/ollama/chat.py`
...
The `Ollama` class is a dataclass that inherits from the `Model` base class and provides a comprehensive interface for interacting with Ollama models. Key features include:

### Attributes
- **Default Configuration**: Uses "llama3.1" as the default model ID
- **Request Parameters**: Support for format, options, keep_alive, and custom request parameters
- **Client Management**: Both synchronous and asynchronous Ollama client support
- **Structured Outputs**: Native support for structured outputs

### Key Methods
- `invoke()` - Synchronous chat requests
- `ainvoke()` - Asynchronous chat requests  
- `invoke_stream()` - Streaming chat requests
- `ainvoke_stream()` - Asynchronous streaming
- `parse_provider_response()` - Parse responses from Ollama
- `parse_provider_response_delta()` - Parse streaming response deltas
- `_format_message()` - Format messages for Ollama API
- `get_client()` / `get_async_client()` - Client management

### Dependencies
- Requires the `ollama` package to be installed
- Uses agno's base Model class and related utilities
- Supports images, tools, and structured outputs

This class provides a complete interface for working with Ollama models within the agno framework.