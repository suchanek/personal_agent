#!/usr/bin/env python3
"""
Comprehensive Streaming Response Analyzer for Agno Agents

This program provides a complete analysis of streaming responses from Agno agents,
with detailed examination of each chunk to find tool call information.
"""

import asyncio
import re
import sys
from pathlib import Path
from typing import Any, Dict, Iterator, List

# Add src to path for imports
sys.path.insert(0, str(Path(__file__).parent))

from agno.agent import Agent, RunResponse
from agno.run.response import RunResponseEvent
from agno.utils.common import dataclass_to_dict

from src.personal_agent.team.reasoning_team import create_image_agent


def collect_and_examine_chunks(
    run_stream: Iterator[RunResponseEvent],
) -> List[Dict[str, Any]]:
    """
    Collect all chunks and examine each one in detail for tool call information.

    Args:
        run_stream: Iterator from agent.run(stream=True)

    Returns:
        List of dictionaries with chunk data and analysis
    """
    chunks_data = []

    for i, chunk in enumerate(run_stream):
        chunk_info = {
            "index": i,
            "chunk_object": chunk,
            "event_type": getattr(chunk, "event", "unknown"),
            "has_content": hasattr(chunk, "content") and chunk.content is not None,
            "content_length": (
                len(str(chunk.content))
                if hasattr(chunk, "content") and chunk.content
                else 0
            ),
            "has_status": hasattr(chunk, "status"),
            "status": str(chunk.status) if hasattr(chunk, "status") else None,
            "has_tools": hasattr(chunk, "tools") and chunk.tools is not None,
            "tools_count": (
                len(chunk.tools) if hasattr(chunk, "tools") and chunk.tools else 0
            ),
            "has_tool": hasattr(chunk, "tool") and chunk.tool is not None,
            "has_extra_data": hasattr(chunk, "extra_data")
            and chunk.extra_data is not None,
            "tool_calls_found": [],
            "image_urls_found": [],
        }

        # Extract tool calls from tools attribute (list)
        if hasattr(chunk, "tools") and chunk.tools:
            for tool in chunk.tools:
                tool_info = {
                    "source": "tools_list",
                    "name": getattr(tool, "tool_name", "Unknown"),
                    "arguments": getattr(tool, "arguments", {}),
                    "status": getattr(tool, "status", "unknown"),
                }
                chunk_info["tool_calls_found"].append(tool_info)

        # Extract tool call from tool attribute (single)
        if hasattr(chunk, "tool") and chunk.tool:
            tool_info = {
                "source": "tool_single",
                "name": getattr(chunk.tool, "tool_name", "Unknown"),
                "arguments": getattr(chunk.tool, "arguments", {}),
                "status": getattr(chunk.tool, "status", "unknown"),
            }
            chunk_info["tool_calls_found"].append(tool_info)

        # Extract image URLs from content
        if hasattr(chunk, "content") and chunk.content:
            image_matches = re.findall(
                r"!\[([^\]]*)\]\((https?://[^\)]+)\)", str(chunk.content)
            )
            for alt_text, url in image_matches:
                chunk_info["image_urls_found"].append(
                    {"alt_text": alt_text, "url": url}
                )

        chunks_data.append(chunk_info)

    return chunks_data


def comprehensive_analysis(chunks_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Perform comprehensive analysis of all collected chunk data.

    Args:
        chunks_data: List of chunk information from collect_and_examine_chunks

    Returns:
        Dictionary with comprehensive analysis results
    """
    analysis = {
        "final_content": "",
        "actual_final_response": "",
        "final_chunk": None,
        "all_tool_calls": [],
        "image_urls": [],
        "status": "unknown",
        "total_chunks": len(chunks_data),
        "chunks_with_content": 0,
        "chunks_with_tools": 0,
        "chunks_with_tool": 0,
        "tool_call_chunks": [],
        "image_url_chunks": [],
    }

    # Process all chunks for comprehensive analysis
    for chunk_info in chunks_data:
        # Count chunks with different attributes
        if chunk_info["has_content"]:
            analysis["chunks_with_content"] += 1
        if chunk_info["has_tools"]:
            analysis["chunks_with_tools"] += 1
        if chunk_info["has_tool"]:
            analysis["chunks_with_tool"] += 1

        # Collect all tool calls
        for tool_call in chunk_info["tool_calls_found"]:
            if tool_call not in analysis["all_tool_calls"]:
                analysis["all_tool_calls"].append(tool_call)

        # Collect all image URLs
        for image_url in chunk_info["image_urls_found"]:
            if image_url not in analysis["image_urls"]:
                analysis["image_urls"].append(image_url)

        # Track chunks with tool calls or image URLs
        if chunk_info["tool_calls_found"]:
            analysis["tool_call_chunks"].append(chunk_info["index"])
        if chunk_info["image_urls_found"]:
            analysis["image_url_chunks"].append(chunk_info["index"])

        # Identify final chunk (usually has completed status)
        if chunk_info["has_status"] and chunk_info["status"] == "RunStatus.completed":
            analysis["final_chunk"] = chunk_info["chunk_object"]
            analysis["status"] = "completed"
            if chunk_info["has_content"]:
                analysis["final_content"] = chunk_info["chunk_object"].content

    # Fallback to last chunk if no completed status found
    if not analysis["final_content"] and chunks_data:
        last_chunk_info = chunks_data[-1]
        analysis["final_chunk"] = last_chunk_info["chunk_object"]
        if last_chunk_info["has_content"]:
            analysis["final_content"] = last_chunk_info["chunk_object"].content
        if last_chunk_info["has_status"]:
            analysis["status"] = last_chunk_info["status"]

    # Extract actual final response (content after </think> tags)
    if analysis["final_content"]:
        content_str = str(analysis["final_content"])
        if '</think>' in content_str:
            parts = content_str.split('</think>')
            if len(parts) > 1:
                actual_response = parts[-1].strip()
                if actual_response:
                    analysis["actual_final_response"] = actual_response

    return analysis


def extract_tool_call_details(chunks_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Extract detailed tool call information including results from chunks.

    Args:
        chunks_data: List of chunk information

    Returns:
        Dictionary with detailed tool call information
    """
    tool_details = {"tool_calls": [], "tool_results": [], "messages": []}

    for chunk_info in chunks_data:
        chunk = chunk_info["chunk_object"]

        # Extract messages if they exist
        if hasattr(chunk, "messages") and chunk.messages:
            for msg in chunk.messages:
                msg_dict = (
                    dataclass_to_dict(msg)
                    if hasattr(msg, "__dataclass_fields__")
                    else msg
                )
                tool_details["messages"].append(
                    {"chunk_index": chunk_info["index"], "message": msg_dict}
                )

        # Extract tool calls with more detail
        for tool_call in chunk_info["tool_calls_found"]:
            # Try to get more details from the actual chunk object
            if hasattr(chunk, "tool") and chunk.tool:
                tool_obj = chunk.tool
                detailed_call = {
                    "chunk_index": chunk_info["index"],
                    "name": getattr(tool_obj, "tool_name", tool_call["name"]),
                    "arguments": getattr(tool_obj, "arguments", tool_call["arguments"]),
                    "status": getattr(tool_obj, "status", tool_call["status"]),
                    "tool_call_id": getattr(tool_obj, "tool_call_id", None),
                    "result": getattr(tool_obj, "result", None),
                    "error": getattr(tool_obj, "error", None),
                    "raw_tool_object": tool_obj,
                }
                tool_details["tool_calls"].append(detailed_call)
            elif hasattr(chunk, "tools") and chunk.tools:
                for tool_obj in chunk.tools:
                    detailed_call = {
                        "chunk_index": chunk_info["index"],
                        "name": getattr(tool_obj, "tool_name", "Unknown"),
                        "arguments": getattr(tool_obj, "arguments", {}),
                        "status": getattr(tool_obj, "status", "unknown"),
                        "tool_call_id": getattr(tool_obj, "tool_call_id", None),
                        "result": getattr(tool_obj, "result", None),
                        "error": getattr(tool_obj, "error", None),
                        "raw_tool_object": tool_obj,
                    }
                    tool_details["tool_calls"].append(detailed_call)

    return tool_details


def find_actual_final_response(chunks_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Find the actual final response by examining all possible attributes of chunks.
    
    Args:
        chunks_data: List of chunk information
        
    Returns:
        Dictionary with information about where the real final response is located
    """
    response_candidates = []
    
    for chunk_info in chunks_data:
        chunk = chunk_info["chunk_object"]
        
        # Examine all attributes of each chunk
        chunk_attrs = {}
        for attr in dir(chunk):
            if not attr.startswith('_'):
                try:
                    value = getattr(chunk, attr)
                    if not callable(value) and value is not None:
                        chunk_attrs[attr] = value
                except:
                    pass
        
        # Look for potential response fields
        potential_response_fields = [
            'response', 'final_response', 'output', 'result', 
            'answer', 'reply', 'message', 'text', 'body'
        ]
        
        for field in potential_response_fields:
            if field in chunk_attrs:
                response_candidates.append({
                    "chunk_index": chunk_info["index"],
                    "field_name": field,
                    "field_value": chunk_attrs[field],
                    "field_type": type(chunk_attrs[field]).__name__,
                    "field_length": len(str(chunk_attrs[field])) if chunk_attrs[field] else 0
                })
        
        # Also check if there are any non-thinking content
        if hasattr(chunk, 'content') and chunk.content:
            content_str = str(chunk.content)
            # Check if content has non-thinking parts
            if not content_str.strip().startswith('<think>') or '</think>' in content_str:
                # Extract content after </think> tags
                parts = content_str.split('</think>')
                if len(parts) > 1:
                    final_part = parts[-1].strip()
                    if final_part and final_part != content_str.strip():
                        response_candidates.append({
                            "chunk_index": chunk_info["index"],
                            "field_name": "content_after_think",
                            "field_value": final_part,
                            "field_type": "extracted_content",
                            "field_length": len(final_part)
                        })
        
        # Store all chunk attributes for debugging
        response_candidates.append({
            "chunk_index": chunk_info["index"],
            "field_name": "all_attributes",
            "field_value": chunk_attrs,
            "field_type": "debug_info",
            "field_length": len(chunk_attrs)
        })
    
    return {
        "response_candidates": response_candidates,
        "total_candidates": len([c for c in response_candidates if c["field_name"] != "all_attributes"])
    }


def print_detailed_findings(
    chunks_data: List[Dict[str, Any]], analysis: Dict[str, Any]
):
    """
    Print detailed findings from the comprehensive analysis.

    Args:
        chunks_data: List of chunk information
        analysis: Analysis results from comprehensive_analysis
    """
    print("🔍 DETAILED CHUNK ANALYSIS")
    print("=" * 40)

    print(f"Total chunks processed: {analysis['total_chunks']}")
    print(f"Chunks with content: {analysis['chunks_with_content']}")
    print(f"Chunks with tools list: {analysis['chunks_with_tools']}")
    print(f"Chunks with single tool: {analysis['chunks_with_tool']}")

    # Extract detailed tool call information
    tool_details = extract_tool_call_details(chunks_data)

    # Show detailed tool call information
    if tool_details["tool_calls"]:
        print(
            f"\n🔧 DETAILED TOOL CALL ANALYSIS ({len(tool_details['tool_calls'])} calls):"
        )
        print("=" * 50)
        for i, tool_call in enumerate(tool_details["tool_calls"], 1):
            print(f"\n   📞 TOOL CALL #{i}")
            print(f"   ├─ Chunk Index: #{tool_call['chunk_index']}")
            print(f"   ├─ Tool Name: {tool_call['name']}")
            print(f"   ├─ Status: {tool_call['status']}")
            print(f"   ├─ Tool Call ID: {tool_call.get('tool_call_id', 'N/A')}")

            if tool_call["arguments"]:
                print(f"   ├─ Arguments: {tool_call['arguments']}")

            if tool_call.get("result"):
                print(f"   ├─ Result: {tool_call['result']}")

            if tool_call.get("error"):
                print(f"   ├─ Error: {tool_call['error']}")

            # Show all attributes of the raw tool object
            if tool_call.get("raw_tool_object"):
                tool_obj = tool_call["raw_tool_object"]
                print(f"   └─ Raw Tool Object Attributes:")
                for attr in dir(tool_obj):
                    if not attr.startswith("_"):
                        try:
                            value = getattr(tool_obj, attr)
                            if not callable(value):
                                print(f"      • {attr}: {value}")
                        except:
                            pass

    # Show messages if found
    if tool_details["messages"]:
        print(f"\n💬 MESSAGES FOUND ({len(tool_details['messages'])} messages):")
        print("=" * 40)
        for i, msg_info in enumerate(tool_details["messages"], 1):
            print(f"\n   📨 MESSAGE #{i} (Chunk #{msg_info['chunk_index']})")
            msg = msg_info["message"]

            # Pretty print the message structure
            if isinstance(msg, dict):
                for key, value in msg.items():
                    if key == "tool_calls" and value:
                        print(f"   ├─ {key}:")
                        for tc in value:
                            print(f"   │  └─ {tc}")
                    elif key == "content" and value and len(str(value)) > 100:
                        print(f"   ├─ {key}: {str(value)[:100]}...")
                    else:
                        print(f"   ├─ {key}: {value}")

    # Show actual final response (extracted from content after </think>)
    if analysis["actual_final_response"]:
        print(f"\n🎯 ACTUAL FINAL AGENT RESPONSE:")
        print("=" * 35)
        print(f"Status: {analysis['status']}")
        print(f"Response Length: {len(str(analysis['actual_final_response']))} characters")
        print(f"\n✨ ACTUAL FINAL RESPONSE (What users see):")
        print("-" * 45)
        print(analysis["actual_final_response"])
        print("-" * 45)
    
    # Show raw final content for comparison
    if analysis["final_content"]:
        print(f"\n🔍 RAW FINAL CONTENT (Including thinking):")
        print("=" * 40)
        print(f"Content Length: {len(str(analysis['final_content']))} characters")
        print(f"\n📝 RAW CONTENT (With <think> tags):")
        print("-" * 35)
        # Show first 200 chars and last 200 chars to see structure
        content_str = str(analysis["final_content"])
        if len(content_str) > 400:
            print(f"{content_str[:200]}...")
            print("...")
            print(f"...{content_str[-200:]}")
        else:
            print(content_str)
        print("-" * 35)

    # Show image URL findings
    if analysis["image_urls"]:
        print(f"\n🖼️  IMAGE URLS FOUND ({len(analysis['image_urls'])}):")
        print("-" * 25)
        for i, image_url in enumerate(analysis["image_urls"], 1):
            print(f"   {i}. Alt text: '{image_url['alt_text']}'")
            print(f"      URL: {image_url['url']}")

    # Show chunks where tool calls were found
    if analysis["tool_call_chunks"]:
        print(f"\n📦 CHUNKS CONTAINING TOOL CALLS:")
        print("-" * 35)
        for chunk_idx in analysis["tool_call_chunks"]:
            print(f"   Chunk #{chunk_idx}")

    # Show chunks where image URLs were found
    if analysis["image_url_chunks"]:
        print(f"\n📦 CHUNKS CONTAINING IMAGE URLS:")
        print("-" * 35)
        for chunk_idx in analysis["image_url_chunks"]:
            print(f"   Chunk #{chunk_idx}")


async def main():
    """Main function demonstrating comprehensive chunk analysis."""
    print("🔍 COMPREHENSIVE STREAMING RESPONSE ANALYZER")
    print("=" * 50)
    print("This program provides detailed analysis of each chunk")
    print("to find tool call information in streaming responses.\n")

    # Create image agent for testing
    image_agent = create_image_agent(debug=False, use_remote=False)
    prompt = "Create an image of a cyberpunk cityscape"

    print(f"Prompt: '{prompt}'")

    # Streaming analysis with comprehensive chunk examination
    print(f"\n1️⃣  COMPREHENSIVE CHUNK EXAMINATION")
    print("-" * 40)

    try:
        # Get streaming iterator
        run_stream: Iterator[RunResponse] = image_agent.run(
            prompt, stream=True, stream_intermediate_steps=True
        )

        # Collect and examine ALL chunks in detail
        print("📦 Collecting and examining ALL chunks...")
        chunks_data = collect_and_examine_chunks(run_stream)
        print(f"✅ Examined {len(chunks_data)} chunks in detail")

        # Perform comprehensive analysis
        print("🧠 Performing comprehensive analysis...")
        analysis = comprehensive_analysis(chunks_data)

        # Display detailed findings
        print_detailed_findings(chunks_data, analysis)
        
        # Find actual final response
        print(f"\n🔍 SEARCHING FOR ACTUAL FINAL RESPONSE:")
        print("=" * 45)
        response_search = find_actual_final_response(chunks_data)
        
        print(f"Found {response_search['total_candidates']} potential response candidates")
        
        # Show non-debug candidates
        non_debug_candidates = [c for c in response_search['response_candidates'] 
                               if c['field_name'] != 'all_attributes']
        
        if non_debug_candidates:
            print(f"\n🎯 POTENTIAL FINAL RESPONSE CANDIDATES:")
            print("-" * 40)
            for i, candidate in enumerate(non_debug_candidates, 1):
                print(f"\n   📋 CANDIDATE #{i}")
                print(f"   ├─ Chunk Index: #{candidate['chunk_index']}")
                print(f"   ├─ Field Name: {candidate['field_name']}")
                print(f"   ├─ Field Type: {candidate['field_type']}")
                print(f"   ├─ Content Length: {candidate['field_length']} characters")
                
                if candidate['field_length'] > 0 and candidate['field_length'] < 500:
                    print(f"   └─ Content Preview: {str(candidate['field_value'])[:200]}...")
                elif candidate['field_length'] > 0:
                    print(f"   └─ Content Preview: {str(candidate['field_value'])[:100]}...")
        
        # Show chunk attributes for last few chunks to debug
        print(f"\n🔧 DEBUGGING: LAST 3 CHUNKS ATTRIBUTES:")
        print("-" * 40)
        debug_candidates = [c for c in response_search['response_candidates'] 
                           if c['field_name'] == 'all_attributes']
        
        for candidate in debug_candidates[-3:]:  # Show last 3 chunks
            print(f"\n   🔍 CHUNK #{candidate['chunk_index']} ATTRIBUTES:")
            attrs = candidate['field_value']
            for attr_name, attr_value in attrs.items():
                if attr_name not in ['content']:  # Skip content as we already show it
                    attr_str = str(attr_value)
                    if len(attr_str) > 100:
                        print(f"   ├─ {attr_name}: {attr_str[:100]}...")
                    else:
                        print(f"   ├─ {attr_name}: {attr_str}")

        # Summary
        print(f"\n📊 FINAL SUMMARY:")
        print("-" * 20)
        print(f"Final status: {analysis['status']}")
        print(f"Final content length: {len(str(analysis['final_content']))} characters")
        print(f"Unique tool calls found: {len(analysis['all_tool_calls'])}")
        print(f"Image URLs found: {len(analysis['image_urls'])}")
        print(f"Response candidates found: {response_search['total_candidates']}")

    except Exception as e:
        print(f"❌ Error in comprehensive analysis: {e}")
        import traceback

        traceback.print_exc()

    print("\n" + "=" * 50)
    print("✅ COMPREHENSIVE ANALYSIS COMPLETE")
    print("=" * 50)
    print("Key findings:")
    print(
        "• Tool calls can appear in both 'tools' (list) and 'tool' (single) attributes"
    )
    print("• Image URLs are embedded in content as markdown")
    print("• Different chunks contain different types of information")
    print("• The final response is in the last chunk or completed status chunk")


if __name__ == "__main__":
    asyncio.run(main())
