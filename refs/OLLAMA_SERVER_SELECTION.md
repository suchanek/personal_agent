...
To run the agent with a remote Ollama server (on 100.115.62.30):
...
To run the web interface with a remote Ollama server (on 100.115.62.30):
...
The remote Ollama URL is configured to use `http://100.115.62.30:11434` when the `--remote-ollama` flag is specified. The local Ollama URL is determined by the `OLLAMA_URL` environment variable, which defaults to `http://localhost:11434` if not set.
