...
class EricMemoryTester:
        ...
        if self.use_temp_db and os.path.exists(self.db_path):
            ...
            try:
                ...
                for memory in memories:
                    self.memory.delete_user_memory(memory.memory_id, user_id=self.test_user_id)
                print(f"🧹 Cleaned up {len(memories)} memories for user {self.test_user_id}")
            except Exception as e:
    ...
    def add_facts_about_eric(self):
        ...
        facts = [
            ...
            "Eric is a biophysicist engineer who specializes in AI and machine learning.",
        ...
        for i, fact in enumerate(facts, 1):
            ...
            memories = self.memory.create_user_memories(message=fact, user_id=self.test_user_id)
            if memories:
    ...
    def analyze_results(self, created_memories: List[UserMemory]):
        ...
        query_terms = ["python", "work schedule", "programming", "interests", "preferences"]
    ...
    def run_test(self):
        ...
        try:
            ...
            if initial_memories:
                print(f"Found {len(initial_memories)} existing memories, cleaning up...")
                for mem in initial_memories:
                    self.memory.delete_user_memory(mem.memory_id, user_id=self.test_user_id)
