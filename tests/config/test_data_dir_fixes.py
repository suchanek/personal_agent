    ...
    try:
        ...
        for var in user_specific_vars:
            ...
            if not any(
                ...
                for user_id in [
                    "default_user",
                    os.environ.get("USER_ID", "default_user"),
                ]
            ):
            ...
            if args and len(args) > 2:
                ...
                if hasattr(settings, "USER_DATA_DIR"):
                    ...
                    if server_path == expected_path:
                        ...
                        print(
                            f"   ❌ MCP server path mismatch: {server_path} != {expected_path}"
                        )
                        return False
                else:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(settings, "AGNO_STORAGE_DIR", temp_dir):
                ...
                if expected_db_path.exists():
                    ...
                    print(
                        f"   ❌ Storage not created at expected path: {expected_db_path}"
                    )
                    return False
...
def test_user_isolation():
    ...
    try:
        from src.personal_agent.config.user_id_mgr import (
    ...
    try:
        ...
        for path_name, path in user_paths:
            ...
            if current_user_id not in path:
                issues.append(
                    f"{path_name} does not contain user ID '{current_user_id}': {path}"
                )
            # Check that it's not using raw DATA_DIR concatenation
            elif (
                path == f"{settings.DATA_DIR}/agno"
                or path == f"{settings.DATA_DIR}/knowledge"
            ):
                issues.append(
                    f"{path_name} appears to use raw DATA_DIR concatenation: {path}"
                )
