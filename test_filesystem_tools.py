...
async def test_filesystem_tools():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_content = (
                "This is a test file created by agno filesystem tools!\nTimestamp: "
                + str(asyncio.get_event_loop().time())
            )

            if "create_file" in filesystem_tools:
                try:
                    result = filesystem_tools["create_file"].entrypoint(
            ...
            write_content = (
                "Content written directly with write_file tool.\nLine 2 of content."
            )

            if "write_file" in filesystem_tools:
                try:
                    result = filesystem_tools["write_file"].entrypoint(
                        write_test_path, write_content
                    )
                    print(f"Result: {result}")
            ...
            if "read_file" in filesystem_tools:
                try:
                    result = filesystem_tools["read_file"].entrypoint(
