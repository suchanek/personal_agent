#!/usr/bin/env python3
"""
Script to verify and update Llama model context sizes using ollama show command.

This script checks all Llama models in the MODEL_CONTEXT_SIZES dictionary
...
def get_llama_models() -> Dict[str, int]:
    """
    Get all Llama models from the MODEL_CONTEXT_SIZES dictionary.
    
    Returns:
        Dictionary of Llama model names and their configured context sizes
    """
    llama_models = {}
    for model_name, context_size in MODEL_CONTEXT_SIZES.items():
        if 'llama' in model_name.lower():
            llama_models[model_name] = context_size
    return llama_models
...
def normalize_model_name_for_comparison(model_name: str) -> str:
    ...
    """
    # Handle case differences (e.g., llama3.1:8B vs llama3.1:8b)
    normalized = model_name.lower()
    
    # Handle variations like llama3.1:8b vs llama3.1:8B
...
def find_matching_available_model(config_model: str, available_models: list) -> Optional[str]:
    ...
    for available in available_models:
        ...
        # Handle B vs b differences (e.g., llama3.1:8B vs llama3.1:8b)
        if config_normalized.replace('b', 'B') == available_normalized.replace('b', 'B'):
            return available
        
        # Handle tag variations (e.g., llama3.1:8b vs llama3.1:8b-instruct-q8_0)
        if config_normalized in available_normalized or available_normalized in config_normalized:
            return available
...
def main():
    """Main function to verify Llama model context sizes."""
    print("🔍 Verifying Llama model context sizes...")
    print("=" * 60)
    ...
    # Get Llama models from our configuration
    llama_models = get_llama_models()
    print(f"📊 Found {len(llama_models)} Llama models in configuration")
    
    # Check each Llama model
    discrepancies = []
    verified_models = []
    unavailable_models = []
    
    for model_name, configured_context in llama_models.items():
        print(f"\n🔍 Checking {model_name}...")
    ...
    if not discrepancies and verified_models:
        print("\n🎉 All available Llama models have correct context sizes!")
