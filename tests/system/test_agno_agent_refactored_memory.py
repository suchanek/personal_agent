...
from personal_agent.config.user_id_mgr import get_user_id
...
async def test_refactored_agent_memory():
    ...
    direct_results = agent._direct_search_memories(
        query="testing refactored agent", limit=5, similarity_threshold=0.3
    )
    ...
    response3 = await agent.run(
        "Please remember that I prefer direct semantic search over agentic retrieval"
    )
...
async def test_memory_search_priority():
    ...
    direct_results = agent._direct_search_memories(
        query="semantic search", limit=5, similarity_threshold=0.3
    )
