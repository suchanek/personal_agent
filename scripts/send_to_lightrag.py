...
def send_file_to_lightrag(file_path: str, remote_host: str = None, remote_user: str = None, remote_path: str = None, remote_lightrag_url: str = None):
    ...
    if remote_host and remote_user and remote_path:
        ...
        try:
            ...
            print(f"Error: 'scp' command not found. Make sure OpenSSH client is installed and in your PATH.")
        return
...
def main():
    parser = argparse.ArgumentParser(description="Send a file to the LightRAG server or copy it via SCP.")
    parser.add_argument("file_path", help="The path to the file to send.")
    parser.add_argument("--remote-host", default="tesla.local", help="Optional: The hostname or IP address of the remote server for SCP transfer (default: tesla.local).")
    parser.add_argument("--remote-user", default="suchanek", help="Optional: The SSH username for the remote server (default: suchanek).")
    parser.add_argument("--remote-path", default="/Users/Shared/personal_agent_data/knowledge", help="Optional: The absolute path on the remote server where the file should be copied (default: /Users/Shared/personal_agent_data/knowledge).")
    parser.add_argument("--remote-lightrag-url", default="http://tesla.local:9621", help="Optional: The URL of the remote LightRAG server API to trigger rescan (default: http://tesla.local:9621).")
    args = parser.parse_args()

    send_file_to_lightrag(args.file_path, args.remote_host, args.remote_user, args.remote_path, args.remote_lightrag_url)
