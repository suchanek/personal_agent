...
show_usage() {
    ...
    echo "  remote  - Switch to remote Ollama server (100.115.62.30:11434)"
...
show_status() {
    ...
    if [[ "$current_url" == "$LOCAL_URL" ]]; then
        ...
        echo -e "  RAG Mode: ${GREEN}REMOTE${NC} (100.115.62.30)"
    else
...
restart_docker_services() {
    ...
    if [ -f ".env" ]; then
        ...
        elif [[ "$current_url" == *"100.115.62.30"* ]]; then
...
case "$1" in
    ...
    "remote")
        echo -e "${BLUE}Switching to REMOTE Ollama server (100.115.62.30)...${NC}"
