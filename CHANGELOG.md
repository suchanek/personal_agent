...
- **Consolidated Knowledge Management**: Refactored and enhanced the `KnowledgeManager` to serve as the single, authoritative interface for all knowledge-related operations, encompassing both semantic and graph knowledge bases. This centralizes query logic, unifies tool interaction, and streamlines knowledge operations, significantly improving modularity, maintainability, and consistency. See [ADR-070](./refs/adr/070-consolidated-knowledge-management.md) for details.
- **Refined Team Memory Handling and Streamlit UI Integration**: Enhanced the `PersonalAgentTeam`'s memory handling by explicitly routing `store_user_memory` operations through the dedicated `Knowledge Agent`, ensuring consistent fact restatement and LLM processing. Implemented a robust fallback mechanism for direct semantic memory storage. The Streamlit UI's memory helper and display logic were updated to gracefully handle both synchronous/asynchronous memory calls and diverse return types, improving overall stability and user experience. This refines the architectural decisions made in [ADR-069](./refs/adr/069-personal-agent-team-refactor.md).
