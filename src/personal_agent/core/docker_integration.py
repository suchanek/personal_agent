...
class DockerIntegrationManager:
    ...
    def check_docker_consistency(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if all_consistent:
                return (
                    True,
                    f"All Docker configurations consistent with USER_ID: {self.user_id}",
                )
            else:
                ...
                return (
                    False,
                    f"Inconsistent Docker servers: {', '.join(inconsistent_servers)}",
                )
    ...
    def ensure_docker_consistency(
        ...
        try:
            # Caller has already checked consistency - proceed directly to sync
            logger.info(
                "Synchronizing Docker configurations with USER_ID: %s", self.user_id
            )

            # Perform synchronization (docker_sync will do its own check internally)
    ...
    def pre_initialization_check(
        self, auto_fix: bool = True, force_restart: bool = False
    ) -> Tuple[bool, str]:
        """Perform pre-initialization consistency check and optional auto-fix.

        This method performs a single consistency check and delegates fixing to
        ensure_docker_consistency() if needed. This avoids redundant checks.

        Flow:
        1. Check consistency once
        2. If inconsistent or force_restart, call ensure_docker_consistency()
        3. ensure_docker_consistency() calls sync_user_ids() which does its own internal check
        ...
        """
        logger.info(
            "Performing pre-initialization Docker consistency check for USER_ID: %s",
            self.user_id,
        )
        ...
        if is_consistent and not force_restart:
            logger.info(
                "Pre-initialization check passed - Docker configurations consistent"
            )
            ...
            if auto_fix:
                ...
                fix_success, fix_message = self.ensure_docker_consistency(
                    force_restart=True
                )
                if fix_success:
                ...
                return (
                    True,
                    f"Consistent but force restart requested (auto-fix disabled): {check_message}",
                )

        logger.warning("Pre-initialization check failed: %s", check_message)

        if not auto_fix:
            return (
                False,
                f"Docker inconsistency detected (auto-fix disabled): {check_message}",
            )
...
def ensure_docker_user_consistency(
    user_id: Optional[str] = None, auto_fix: bool = True, force_restart: bool = False
) -> Tuple[bool, str]:
    ...
    try:
        ...
        return manager.pre_initialization_check(
            auto_fix=auto_fix, force_restart=force_restart
        )
    except Exception as e:
