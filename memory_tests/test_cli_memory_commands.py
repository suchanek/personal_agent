...
async def run_tests():
    ...
    agent, _ = await initialize_agno_system(use_remote_ollama=False, recreate=False)
    ...
    try:
        ...
        if store_memory_func:
            ...
            await agent.store_user_memory(
                content=test_memory_with_topic, topics=[test_topic]
            )
