...
class AgentMemoryManager:
    ...
    def __init__(self, user_id: str, storage_dir: str, agno_memory=None,
                 lightrag_url: Optional[str] = None, lightrag_memory_url: Optional[str] = None,
    ...
    def direct_search_memories(self, query: str, limit: int = 10, similarity_threshold: float = 0.3):
    ...
    async def get_memory_tools(self) -> List:
        ...
        async def query_graph_memory_tool(
            query: str, mode: str = "mix", top_k: int = 5, response_type: str = "Multiple Paragraphs"
        ) -> dict:
    ...
    async def store_user_memory(self, content: str = "", topics: Union[List[str], str, None] = None) -> MemoryStorageResult:
                            ...
                            if docs_to_delete:
                                ...
                                async with session.delete(
                                    ...
                                    if del_resp.status == 200:
                                        ...
                                        results.append("✅ Graph memory: Deleted successfully")
                                    else:
                                        error_text = await del_resp.text()
    ...
    async def get_recent_memories(self, limit: int = 10) -> str:
        ...
        try:
            ...
            logger.info("Retrieved %d recent memories for user %s", len(display_memories), self.user_id)
            return result
    ...
    async def get_all_memories(self) -> str:
        ...
        try:
            ...
            logger.info("Retrieved all %d memories for user %s", len(memories), self.user_id)
            return result
    ...
    async def get_memory_stats(self) -> str:
        ...
        try:
            ...
            if newest_timestamp:
                from datetime import datetime
                newest_date = datetime.fromtimestamp(newest_timestamp).strftime('%Y-%m-%d %H:%M:%S')
    ...
    async def get_memories_by_topic(self, topics: Union[List[str], str, None] = None, limit: Union[int, None] = None) -> str:
        ...
        try:
            ...
            if topics is None:
                ...
                return "❌ Error: No topics provided. Please specify at least one topic."
            ...
            for memory in all_memories:
                if hasattr(memory, 'topics') and memory.topics:
                    ...
                    if any(topic.lower() in [t.lower() for t in memory.topics] for topic in topic_list):
            ...
            logger.info("Retrieved %d memories for topics %s", len(display_memories), topics_str)
            return result
    ...
    async def list_memories(self) -> str:
        ...
        try:
            ...
            for i, memory in enumerate(sorted_memories, 1):
                ...
                memory_preview = memory.memory[:100] + "..." if len(memory.memory) > 100 else memory.memory
            ...
            logger.info("Listed all %d memories for user %s in simplified format", len(memories), self.user_id)
            return result
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    with open(temp_file_path, 'rb') as file:
                        ...
                        async with session.post(url, data=data, timeout=30) as resp:
                            if resp.status in [200, 201]:
                                logger.info("Successfully stored memory in graph database: %s", filename)
                                return f"✅ Memory successfully stored in graph database: {content[:50]}..."
                            else:
                                error_detail = await resp.text()
                                logger.warning(f"Failed to store memory in graph: {error_detail}")
                                return f"❌ Error storing memory in graph: {error_detail}"
    ...
    async def query_graph_memory(self, query: str, mode: str = "mix", top_k: int = 5, response_type: str = "Multiple Paragraphs") -> dict:
        ...
        try:
            if not self.lightrag_memory_url:
                logger.warning("LightRAG memory URL not configured")
                return {"error": "Graph memory not configured. Cannot query graph database."}
            ...
            valid_response_types = ["Multiple Paragraphs", "Single Paragraph", "Bullet Points", "JSON"]
            if response_type not in valid_response_types:
                logger.warning(f"Invalid response_type '{response_type}' provided to query_graph_memory")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params, timeout=60) as resp:
                    if resp.status == 200:
                        result = await resp.json()
                        logger.info(f"Successfully queried graph memory: {query[:50]}...")
                        return result
                    else:
                        error_detail = await resp.text()
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=30) as resp:
                    if resp.status == 200:
                        ...
                        error_detail = await resp.text()
                        logger.warning(f"Failed to retrieve graph labels: {error_detail}")
