...
def validate_user_id(user_id: str) -> bool:
    ...
    if not user_id.replace('_', '').replace('-', '').isalnum():
        print_error("User ID can only contain letters, numbers, hyphens, and underscores")
        return False
...
def create_user_if_needed(user_id: str, user_name: str = None, user_type: str = "Standard") -> bool:
    ...
    try:
        ...
        if result["success"]:
            print_success(f"Created user '{user_id}' with name '{user_name}' (type: {user_type})")
            return True
        else:
    ...
    try:
        ...
        if result["success"]:
            ...
            if result.get("config_refresh"):
                ...
                print(f"  • Memory Storage: {config_refresh.get('LIGHTRAG_MEMORY_STORAGE_DIR', 'N/A')}")
            ...
            if restart_services and result.get("lightrag_status"):
                ...
                if lightrag_status.get("success"):
                    ...
                    if lightrag_status.get("services_restarted"):
                        print_info(f"Services restarted: {', '.join(lightrag_status['services_restarted'])}")
                else:
    ...
    try:
        ...
        try:
            ...
            if directories_created:
                print_success(f"Created {len(directories_created)} new user directories:")
                for dir_path in directories_created:
                    print(f"  • {dir_path}")
            
            if directories_existing:
                print_info(f"Verified {len(directories_existing)} existing user directories:")
...
def display_user_status(user_id: str):
    ...
    try:
        ...
        if user_details:
            ...
            print(f"  • Is Current: {Colors.GREEN if user_details.get('is_current') else Colors.RED}{user_details.get('is_current', False)}{Colors.NC}")
...
Examples:
  ...
  python switch-user.py --status alice
    ...
    parser.add_argument(
        "user_id",
        help="User ID to switch to"
    )
    
    parser.add_argument(
        "--user-name",
        help="Display name for new users (defaults to formatted user_id)"
    )
    ...
    parser.add_argument(
        "--status",
        action="store_true",
        help="Display user status instead of switching"
    )
    ...
    print(f"{Colors.CYAN}You can now use the Personal Agent system as '{args.user_id}'.{Colors.NC}")
    
    if restart_services:
        print(f"{Colors.CYAN}LightRAG services have been restarted with the new user context.{Colors.NC}")
    else:
        print(f"{Colors.YELLOW}Note: LightRAG services were not restarted. You may need to restart them manually.{Colors.NC}")
