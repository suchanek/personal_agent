...
async def test_memory_agent(model_provider: str, model_name: str):
    ...
    if hasattr(agent.agent, 'tools') and agent.agent.tools:
        memory_tools = [tool for tool in agent.agent.tools if hasattr(tool, '__name__') and 'memory' in tool.__name__.lower()]
    ...
    response1 = await agent.run("My name is Eric and I live in San Francisco. Please remember this information about me.")
    print(f"Response: {response1}")
    
    # Test 2: Store another memory
    print(f"\n🧠 Test 2: Storing memory about user's preferences...")
    response2 = await agent.run("I really enjoy hiking in the mountains on weekends. Please store this as a memory.")
    print(f"Response: {response2}")
    
    # Test 3: Try to store duplicate information
    print(f"\n🧠 Test 3: Attempting to store duplicate...")
    response3 = await agent.run("My name is Eric and I live in San Francisco. Store this please.")
    ...
    response5 = await agent.run("What do I like to do on weekends? Look in your memory.")
...
async def main():
    ...
    # Test with Ollama (the problematic one)
    await test_memory_agent("ollama", "qwen2.5:1.5b")
