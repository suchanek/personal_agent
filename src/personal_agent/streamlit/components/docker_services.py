...
from personal_agent.streamlit.utils.smart_docker_restart import (
    get_smart_restart_manager,
)
...
def docker_services_tab():
    ...
    try:
        ...
        if docker_client is None:
            # Docker not accessible - show informational message
            st.warning(
                "⚠️ **Docker Management Not Available**\n\n"
                "The Docker daemon is not accessible from this session. This could be because:\n\n"
                "1. Docker Desktop is not running\n"
                "2. Permission issues with the Docker socket\n"
                "3. Running as a different user than Docker Desktop\n\n"
                "**To enable Docker management:**\n"
                "- Ensure Docker Desktop is running\n"
                "- Check Docker socket permissions\n"
                "- Consider running the dashboard with appropriate permissions\n\n"
                "**Alternative:** Use command-line tools:\n"
                "```bash\n"
                "# Smart restart LightRAG services\n"
            ...
            st.info(f"Currently running as user: **{current_user}**")

            # Show Docker socket info
            st.code("Docker socket: /var/run/docker.sock", language="bash")

            return

    except ImportError as e:
        st.error(f"Docker Python SDK not available: {str(e)}")
        st.info("Install Docker SDK with: `pip install docker`")
        return
    except Exception as e:
        st.error(f"Unexpected error checking Docker: {str(e)}")
        return

    # Docker is accessible - show full management interface
...
def _render_container_management():
    ...
    try:
        ...
        if containers:
                ...
                if st.button("Start", disabled=selected_status == "running"):
                    try:
                        if start_container(selected_container):
                            st.success(
                                f"Container '{selected_container}' started successfully!"
                            )
                            st.rerun()
                        else:
                            st.error(
                                f"Failed to start container '{selected_container}'."
                            )
                    except Exception as e:
                ...
                if st.button("Stop", disabled=selected_status != "running"):
                    try:
                        if stop_container(selected_container):
                            st.success(
                                f"Container '{selected_container}' stopped successfully!"
                            )
                            st.rerun()
                        else:
                            st.error(
                                f"Failed to stop container '{selected_container}'."
                            )
                    except Exception as e:
                ...
                if st.button("Restart"):
                    try:
                        if restart_container(selected_container):
                            st.success(
                                f"Container '{selected_container}' restarted successfully!"
                            )
                            st.rerun()
                        else:
                            st.error(
                                f"Failed to restart container '{selected_container}'."
                            )
                    except Exception as e:
            ...
            st.info(
                "🧠 Smart restart includes proper port cleanup and waiting periods to prevent 'port already allocated' errors."
            )
            ...
            with col1:
                if st.button(
                    "🔄 Smart Restart Selected",
                    help="Intelligently restart the selected container with proper cleanup",
                ):
                    try:
                        ...
                        if service_name:
                            with st.spinner(
                                f"Performing smart restart of {service_name}..."
                            ):
                                success, message = smart_restart.smart_restart_service(
                                    service_name
                                )
                            ...
                            st.warning(
                                f"Smart restart not available for container: {selected_container}"
                            )
                    except Exception as e:
            ...
            with col2:
                if st.button(
                    "🔄 Smart Restart All",
                    help="Intelligently restart all LightRAG services with proper cleanup",
                ):
                    try:
                        smart_restart = get_smart_restart_manager()
                        with st.spinner("Performing smart restart of all services..."):
                            success, message = (
                                smart_restart.smart_restart_all_services()
                            )
            ...
            with col3:
                if st.button(
                    "🔧 Force User Sync",
                    help="Force restart containers with USER_ID synchronization",
                ):
                    try:
                        with st.spinner("Performing Docker user synchronization..."):
                            success, message = (
                                docker_integration.ensure_docker_consistency(
                                    force_restart=True
                                )
                            )
    ...
    try:
        ...
        if containers:
            ...
            selected_container = st.selectbox(
                "Select Container for Logs", container_names
            )
            ...
            with col2:
                auto_refresh = st.checkbox(
                    "Auto Refresh", value=False, help="Automatically refresh logs"
                )
    ...
    try:
        ...
        if containers:
            ...
            if container_stats:
                ...
                selected_container = st.selectbox(
                    "Select Container for Detailed Stats", container_names
                )
                ...
                selected_stats = next(
                    (
                        ...
                        if stat["Name"] == selected_container
                    ),
                    None,
                )
                ...
                if selected_stats:
                    ...
                    st.info(
                        "Historical performance charts will be implemented in a future update."
                    )
            else:
