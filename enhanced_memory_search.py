...
class EnhancedMemorySearcher:
            ...
            for row in memory_rows:
                if row.user_id == self.user_id and row.memory:
                    try:
                        ...
                        print(f"Warning: Failed to convert memory row to UserMemory: {e}")
    ...
    def search_content_only(self, query: str, memories: List[UserMemory], 
                           similarity_threshold: float = 0.3) -> List[Tuple[UserMemory, float, str]]:
        """Search only in memory content (current Streamlit behavior)."""
        results = []
        
        for memory in memories:
            similarity = self.semantic_manager.duplicate_detector._calculate_semantic_similarity(
                query, memory.memory
            )
    ...
    def search_topics_only(self, query: str, memories: List[UserMemory]) -> List[Tuple[UserMemory, float, str]]:
        ...
        for memory in memories:
            if memory.topics:
                ...
                topic_matches = [topic for topic in memory.topics if query_lower in topic.lower()]
                if topic_matches:
                    ...
                    results.append((memory, final_score, f"topics: {', '.join(topic_matches)}"))
        
        return sorted(results, key=lambda x: x[1], reverse=True)
    
    def search_combined(self, query: str, memories: List[UserMemory], 
                       similarity_threshold: float = 0.3) -> List[Tuple[UserMemory, float, str]]:
        """Search both content and topics, combining scores."""
        content_results = {id(mem): (mem, score, "content") 
                          for mem, score, _ in self.search_content_only(query, memories, similarity_threshold)}
        
        topic_results = {id(mem): (mem, score, match_info) 
            ...
            if memory.topics:
                for topic in memory.topics:
                    ...
                    if len(topic_samples[topic]) < 2:  # Keep max 2 samples per topic
                        topic_samples[topic].append(memory.memory[:100] + "..." if len(memory.memory) > 100 else memory.memory)
    ...
    def search_and_debug(self, query: str, limit: int = 10) -> Dict[str, Any]:
        ...
        if content_results:
            for i, (memory, score, match_info) in enumerate(content_results[:limit], 1):
                ...
                print(f"   Memory: {memory.memory[:100]}{'...' if len(memory.memory) > 100 else ''}")
        ...
        print("-" * 50)
        if combined_results:
            for i, (memory, score, match_info) in enumerate(combined_results[:limit], 1):
                print(f"{i}. Score: {score:.3f} - {match_info}")
                print(f"   Memory: {memory.memory[:100]}{'...' if len(memory.memory) > 100 else ''}")
                print(f"   Topics: {memory.topics if memory.topics else 'None'}")
                print()
        ...
        return {
            ...
            "topic_matches": [(m.memory[:50], s, info) for m, s, info in topic_results[:3]],
            "combined_matches": [(m.memory[:50], s, info) for m, s, info in combined_results[:3]]
        }
...
def main():
    ...
    parser.add_argument("--analyze", "-a", action="store_true", help="Analyze database contents")
    ...
    if args.analyze:
        ...
        print(f"Average content length: {analysis['average_content_length']:.1f} characters")
